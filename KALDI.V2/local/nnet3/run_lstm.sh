...
if [ $stage -le 9 ]; then
  ...
  num_jobs=`cat data/dev_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l`
  steps/nnet3/lstm/decode.sh --nj 8 --cmd "$decode_cmd" --extra-left-context $extra_left_context --extra-right-context $extra_right_context --frames-per-chunk "$frames_per_chunk" exp/tri3b/graph data/dev exp/nnet3/blstm_l3_bidirectional_ld5/decode_dev
fi
