...
online_ivector_dir=
presoftmax_prior_scale_power=-0.25  # we haven't yet used pre-softmax prior scaling in the GRU model
...
chunk_width=20  # number of output labels in the sequence used to train an GRU
...
gru_delay=" -1 -2 -3 "  # the delay to be used in the recurrence of grus
                        ...
                        # "[-1,1] [-2,2] [-3,3]" means a three layer stacked bi-directional GRU would use recurrence
...
if [ $# != 4 ]; then
  ...
  echo "                                                   # the number of fields determines the number of GRU and non-recurrent layers"
  echo "                                                   # also see the --num-gru-layers option"
  ...
  echo "  --label-delay <int|5>                            # the gru output is used to predict the label with the specified delay"

  echo "  --gru-delay <str|\" -1 -2 -3 \">                 # the delay to be used in the recurrence of grus"
  echo "                                                   # \"-1 -2 -3\" means the a three layer stacked GRU would use recurrence connections with "
  echo "                                                   # delays -1, -2 and -3 at layer1 gru, layer2 gru and layer3 gru respectively"
  ...
  echo " ################### GRU specific training options ###################### "
